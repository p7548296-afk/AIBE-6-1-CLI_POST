...
* **역할**: 사용자 인터랙션 담당. `Scanner`를 통해 입력을 받고 `System.out`으로 결과를 출력합니다.
* **특징**: `try-catch`를 통해 서비스 계층에서 넘어온 예외 메시지를 사용자에게 일관된 형식으로 전달합니다.

### 2. ArticleService (Business Logic)
* **역할**: 핵심 비즈니스 로직 수행 및 데이터 검증을 담당합니다.
* **특징**: `getArticle` 호출 시 데이터가 없으면 `RuntimeException`을 던져 방어적으로 설계했습니다. `Optional`을 활용한 선언적 프로그래밍을 적용했습니다.

### 3. ArticleRepository (Data Access)
* **역할**: `HashMap`을 사용한 인메모리 데이터 저장소입니다.
* **특징**: 조회 시 `Optional<Article>`을 반환하여 Null 안정성을 확보하였으며, `findAll` 시 스트림 정렬을 통해 최신순 목록을 제공합니다.

### 4. Rq & TestUtil (Utilities)
* **Rq**: 정규표현식(`\\s+`)과 `trim()`을 사용하여 불규칙한 공백 입력을 처리하고 명령어와 인자를 분리합니다.
...
명령어: list
번호 | 제목           | 등록일
1    | 첫 번째 게시글 | 26-04-14 17:15

명령어: update 1
제목(기존: 첫 번째 게시글): 수정된 제목
내용(기존: 안녕하세요!): 수정된 내용
1번 게시글이 수정되었습니다.
...
### 계층형 아키텍처  적용
* 객체의 책임을 `Controller-Service-Repository`로 분리하여 코드의 유지보수성과 가독성을 극대화했습니다.
