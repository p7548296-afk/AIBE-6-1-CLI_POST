...
public class ArticleService {
    ...
    public Page<Article> getArticles(Pageable pageable) {
        ...
        return pagedList(allArticles, pageable);
    }
    ...
    public Page<Article> getSearchArticles(Pageable pageable, String target, String keyword) {
        List<Article> filteredArticles = articleRepository.findAll().stream()
                .filter(article -> {
                    if (target.equals("title")) return article.getTitle().contains(keyword);
                    if (target.equals("content")) return article.getContent().contains(keyword);
                    return article.getTitle().contains(keyword) || article.getContent().contains(keyword);
                })
                .toList();

        return pagedList(filteredArticles, pageable);
    }

    private Page<Article> pagedList(List<Article> list, Pageable pageable) {
        ...
        List<Article> content = list.subList(fromIndex, toIndex);
